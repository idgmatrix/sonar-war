...
`direct_path_arrival`은 각 대역에 대해 `delay=R/c`, `pressure_gain=10^(−TL/20)`,
`energy_gain=pressure_gain²`인 최소 채널 임펄스를 제공하며, `propagate`도 이 객체의 TL을
사용한다. 현재 실시간 합성은 정상상태 음장을 전제로 공통 `R/c` 지연을 버퍼링하지 않고
배열 내부 상대 지연만 적용한다. 따라서 이 도달 시간은 검증 메타데이터이며, 표적 상태
변화의 retarded-time 처리는 월드→DSP 동적 경계를 추가할 때 연결한다.
...
측정 상선 톤은 keel 방향과 임의 고각에서 자유수면 방향 응답을 자동 검사한다. JOMOPANS
대표 다섯 대역은 중심 PSD `±2.5 dB`와 decidecade 적분 에너지 `±2.5 dB`를 함께
검사하고, 최소 직접 경로 채널은 별도 골든으로 검사한다. AudioWorklet의
